...
int main(){
    while(1){
        gr.clear();con.clear();vis.clear();
        lo n,te;
        string s;
        cin>>n;
        if(n==0)break;
        REP(0,2*n)gr.pb(vl(0,0));
        con.assign(2*n,0);
        vis.assign(2*n,0);
        REP(0,n){
            cin>>te>>s;
            te--;
            if(s=="false"){
                gr[i].pb(n+te);gr[n+te].pb(i);
                gr[te].pb(n+i);gr[n+i].pb(te);
            }else{
                gr[i].pb(te);gr[te].pb(i);
                gr[n+i].pb(n+te);gr[n+te].pb(n+i);
            }
        }
        REP(0,2*n){
            if(!vis[i])dfs(i);
        ...
        lo f=1;
        //REP(0,n)cout<<con[i]<<" "<<con[n+i]<<endl;
        REP(0,n)if(con[i]==con[i+n])f=0;
        if(f)cout<<"NOT PARADOX"<<endl;
